...
Lemma coef_trpoly0 i : (0 : {trpoly R n})`_i = 0.
...
Lemma coef_trpoly1 i : (1 : {trpoly R n})`_i = (i == 0%N)%:R.
...
Lemma coef_trXns f i : (trXns n f)`_i = if i <= n then f`_i else 0.
Proof. by rewrite coef_trXn -coef_trpolyE. Qed.
...
Lemma coef_trpolyX m i :
  (\X : {trpoly R m})`_i = (m != 0%N)%:R * (i == 1%N)%:R.
Proof. by rewrite coef_trpolyE val_trpolyX coefZ coefX. Qed.

Lemma trXn_trpolyX m : @trXn R n (trpoly \Xo(m.+1)) = \X.
Proof.
case: n => [|n'].
  rewrite [RHS]trpoly0X //; apply trpolyP => i.
  rewrite leqn0 => /eqP ->.
  by rewrite coef_trpoly0 coef_trXn -coef_trpolyE coef_trpolyX /= mulr0.
by apply trpoly_inj; rewrite !val_trpolySX.
Qed.

Lemma trXns_trpolyX m : @trXns R _ n \Xo(m.+1) = \X.
...
Proof.
apply/trpolyP => i Hi.
rewrite coef_mulfX coef_trpolyXM coef_trXns Hi.
by case: i Hi => //= i /ltnW ->.
Qed.
...
rewrite coef_trXns !coef_mulfX le_in1.
by case: i le_in1 => //= i; rewrite ltnS coef_trXns => ->.
Qed.

Lemma mulfXM f (g : {trpoly R m.+1}) : (mulfX f) * g = mulfX (f * trXns m g).
Proof.
apply/trpolyP => [[|i]].
  by rewrite !(coef0_trpolyM, coef_mulfX, coef0_trXn, coef_trpolyE) /= mul0r.
rewrite ltnS => le_im.
rewrite coef_mulfX /= -/(_`_i.+1) !coefM_trpoly ltnS le_im.
rewrite big_ord_recl /= -/(_`_0) coef_mulfX /= mul0r add0r.
apply eq_bigr => [[j] /=]; rewrite ltnS => le_ji _.
rewrite -/(_`_j.+1) coef_mulfX /= /bump /= add1n subSS.
by rewrite coef_trXns (leq_trans (leq_subr _ _) le_im).
Qed.
...
apply/trpolyP => i le_in.
by rewrite coef_trpoly0 H0 // (leq_ltn_trans le_in le_nm).
Qed.
...
Fact comp_trpoly_is_linear (f : {trpoly R n}) : linear (comp_trpoly f).
Proof.
...
Qed.
Canonical comp_trpoly_additive f := Additive (comp_trpoly_is_linear f).
Canonical comp_trpoly_linear f := Linear (comp_trpoly_is_linear f).


Lemma comp_trpolyXr f : f \So \X = f.
Proof.
case: n f => [|n'] f.
  by rewrite trpoly0X // comp_trpoly0r {2}(trpoly_is_cst f).
rewrite comp_trpoly_coef0_eq0 ?trpolyX_in_coef0_eq0 //.
by rewrite val_trpolySX comp_polyXr trpolyK.
Qed.

Lemma comp_trpolyX : {in coef0_eq0, forall f, \X \So f = f}.
Proof.
case: n => [|n'] f.
  rewrite trpoly0X // comp_trpoly0l {2}(trpoly_is_cst f) => /eqP ->.
  by rewrite trpolyC0.
move=> /comp_trpoly_coef0_eq0 ->.
...
Variables (R : comRingType) (n : nat).
Implicit Types (f g h : {trpoly R n}) (p : {poly R}).

Fact comp_trpoly_is_rmorphism f : multiplicative (comp_trpoly f).
Proof.
...
Canonical comp_trpoly_rmorphism f := AddRMorphism (comp_trpoly_is_rmorphism f).
Canonical comp_trpoly_lrmorphism f := [lrmorphism of (comp_trpoly f)].
...
Proof.
case: n f => [|n'] f.
  rewrite coef_trpoly [in RHS]coef_trpoly leqn0.
...
by rewrite coef_trXn lt_in1 linearZ /= val_trpolyX scale1r.
Qed.
...
rewrite {}[in LHS]IHo -rmorphX coef_trXns le_io1.
set X :=  (_ ^+ k in RHS); have -> : X`_i = (trXns o.+1 X)`_i.
  by rewrite {}/X coef_trXns le_io1.
...
Lemma lagrfixE (f : {trpoly R n}) : lagrfix f = lagrinv (mulfX f^-1).
...
case: n g => [|n] g gU.
  rewrite /lagrfix /= (trpoly_is_cst g) comp_trpoly0r.
  move: gU; rewrite unit_trpolyE; move: (g`_0) => g0 {g} gU.
  rewrite trXnsC coef_trpolyC /=.
  case: k => [|[|k]].
...
rewrite subnn coef0_trpolyM coef_deriv_trpoly coef_mulfX /= -!/(_`_0) mulr1n.
...
rewrite mulfXE derivM_trpoly deriv_trpolyX mul1r /= deriv_trXns.
...
Proof.
rewrite coef0_eq1E coeftrD coef_trpoly1.
by rewrite coeftrZ coef_trpolyE val_trpolyX coefZ coefX !mulr0 addr0.
Qed.
