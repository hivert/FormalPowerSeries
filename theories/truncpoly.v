...
Proof. by apply/trpolyP => i Hi; rewrite coef_trXn coef0 if_same. Qed.
...
Lemma coef_trpolyM f g (i : nat) :
  (f * g)`_i = if i <= n then \sum_(j < i.+1) f`_j * g`_(i - j) else 0.
Proof. by rewrite val_mul_trpoly coef_trXn coefM. Qed.

Lemma coef_trpolyMr f g (i : nat) :
  (f * g)`_i = if i <= n then \sum_(j < i.+1) f`_(i - j) * g`_j else 0.
Proof. by rewrite val_mul_trpoly coef_trXn coefMr. Qed.
...
Proof.
by apply val_inj; rewrite /= -linearZ /= !val_mul_trpoly -scalerAl linearZ.
Qed.
...
Proof. by move=> a b; apply val_inj; rewrite /= polyC_mul -trXn_mul. Qed.

(** TODO This is a ring morphism *)
Fact coef0_trpolyM f g : (f * g)`_0 = f`_0 * g`_0.
Proof. by rewrite coef_trXn coef0M. Qed.

Fact coef0_trpolyX f i : (f ^+ i)`_0 = f`_0 ^+ i.
Proof.
elim: i => [|i IHi]; first by rewrite !expr0 coef1.
by rewrite !exprS coef0_trpolyM IHi.
Qed.
...
Definition trXns m n (f : {trpoly R m}) := trXn n f.

Variables (m n p : nat).
Implicit Type (f : {trpoly R m}).


Lemma trXnsE f : trXns n f = trXn n f.
Proof. by []. Qed.
...
Example trpolys1 : trXn n (1 : {trpoly R m}) = 1.
Proof. by rewrite rmorph1. Qed.

Example trXnsZ c f : trXn n (c *: f) =  c *: (trXns n f).
Proof. by rewrite linearZ. Qed.
...
rewrite raddf_sum /= (bigD1 (Ordinal lt_jn1)) //=.
rewrite coefD coefZ exp_trpoly_val /= val_trpolyX coef_trXn le_jn.
case: (altP (n =P 0%N)) => /= Hn.
- subst n; rewrite scale0r expr0n coef_sum.
  move: le_jn; rewrite leqn0 => /eqP {-5}->.
  rewrite eqxx coef1 eqxx mulr1 big1 ?addr0 // => [[i /= Hi]] Hneq.
  have {Hneq} : i != j.
    by move: Hneq; apply contra => /eqP Heq; apply/eqP/val_inj => /=.
  by move: lt_jn1 Hi; rewrite !ltnS !leqn0 => /eqP -> /eqP ->.
- rewrite scale1r coefXn eqxx mulr1.
  rewrite coef_sum big1 ?coef0 ?addr0 // => [[i /= Hi]] Hneq.
  have {Hneq} Hneq : i != j.
    by move: Hneq; apply contra => /eqP Heq; apply/eqP/val_inj => /=.
  rewrite coefZ exp_trpoly_val /= val_trpolyX coef_trXn le_jn Hn scale1r.
  by rewrite coefXn eq_sym (negbTE Hneq) mulr0.
Qed.
...
Proof.
apply/trpolyP => i Hi.
rewrite coef_trpolyM coef_map_id0 // coef_trpolyMr Hi.
...
rewrite coef_trpolyM -ltnS Hmn big_ord_recl [val ord0]/= subn0.
...
Lemma trXnV f : m <= n -> trXn m (f^-1) = (trXn m f) ^-1.
...
Proof.
apply/trpolyP => i Hi.
rewrite coef_map /= !coef_trpolyM Hi rmorph_sum.
apply eq_bigr => [] [j]; rewrite /= ltnS => le_ji _.
...
Lemma coef0_is_0_trXns (R : ringType) (n m : nat) (f : {trpoly (R) n}) :
  (trXns m f \in coef0_is_0) = (f \in coef0_is_0).
Proof. by rewrite !coef0_is_0E coef0_trXn. Qed.

Lemma coef0_is_1_trXns (R : ringType) (n m : nat) (f : {trpoly (R) n}) :
  (trXns m f \in coef0_is_1) = (f \in coef0_is_1).
Proof. by rewrite !coef0_is_1E coef0_trXn. Qed.
...
Proof.
move=> f; rewrite !coef0_is_1E coef0_inv; move/eqP ->.
by rewrite invr1.
Qed.
...
Lemma divfXK m : {in coef0_is_0, cancel (@divfX m.+1) (@mulfX m)}.
Proof.
move=> p Hp.
...
Lemma trXn_mulfXE f : trXn m (mulfX f) = \X * f.
Proof.
apply/trpolyP => i Hi.
by rewrite coef_trXn coef_mulfX coef_trpolyMX Hi.
Qed.

Lemma mulfXE f : mulfX f = \X * trXns m.+1 f.
Proof.
apply/trpolyP => i Hi.
rewrite coef_mulfX coef_trpolyMX coef_trXn Hi.
by case: i Hi => //= i /ltnW ->.
Qed.
...
rewrite exprS coef_trpolyM; case leqP => // _.
...
rewrite exprS coef_trpolyM (leq_trans lt_im1 _) //.
...
apply/val_inj; rewrite !deriv_trpolyE deriv_trXn /=.
...
rewrite exprS deriv_trpolyM {}IHk /=.
rewrite -mulrnAr mulrA -exprS mulrC rmorphX /= => [|_]; first exact: leq_pred.
...
move/trXnX_eq0/(_ lt_mn) => <-.
by rewrite rmorphX /= trpolyK.
Qed.

Lemma trpolyMX_eq0 n i j : n < i + j ->
  {in @coef0_is_0 R n &, forall f g, f ^+ i * g ^+ j = 0}.
Proof.
move=> lt_n_addij f g f0 g0.
apply val_inj.
rewrite val_mul_trpoly !val_exp_trpoly trXn_mul.
...
Lemma comp_trpoly_coef0_eq0 m (f g : {trpoly R m}) :
  g \in coef0_is_0 -> f \So g = trXn m (f \Po g).
...
Lemma coef_comp_trpoly k f g :
  g \in coef0_is_0 -> (f \So g)`_k = \sum_(i < k.+1) f`_i * (g ^+ i)`_k.
Proof.
...
- rewrite coef_comp_poly.
  have co_is0 i : minn (size f) k.+1 <= i -> f`_i * (g ^+ i)`_k = 0.
    ...
    rewrite !val_exp_trpoly coef_trXn le_kn.
  ...
  rewrite [X in _ + X]big1 ?addr0; first last.
    ...
    by rewrite !val_exp_trpoly coef_trXn le_kn.
  rewrite [X in _ + X]big1 ?addr0; first last.
    by move=> [i /=] _; rewrite -leqNgt => /co_is0.
  rewrite -(big_ord_widen _ (fun i => f`_i * (g ^+ i)`_k)) ?geq_minr //.
  rewrite -(big_ord_widen _ (fun i => f`_i * (val g ^+ i)`_k)) ?geq_minl //.
  by apply eq_bigr => i _; rewrite !val_exp_trpoly coef_trXn le_kn.
- apply/esym/big1 => [[l /=]]; rewrite ltnS => Hl _.
  have := coef_trXn n (g ^+ l) k.
  rewrite -/(trXns _ _) trXns_id leqNgt lt_nk => -> /=.
  by rewrite mulr0.
Qed.
...
Proof.
case (boolP (g \in coef0_is_0)) => [g0_eq0 | g0_neq0] le_mn.
- rewrite !comp_trpoly_coef0_eq0 ?coef0_is_0_trXns //.
  rewrite /trXns -trXn_comp_polyr -trXn_comp_polyl ?(eqP g0_eq0) //.
  by rewrite trXn_trXn.
- rewrite !comp_trpoly_coef0_neq0 ?coef0_is_0_trXns //.
...
by rewrite coef0_is_0E coef0_trXn coefC eqxx negbT.
Qed.

(* TODO : use the coef trick to add canonical structures *)
Fact comp_trpoly_is_additive :
  {in @coef0_is_0 R n, forall f, additive (comp_trpoly f)}.
Proof.
move=> f f0_is_0 u v; rewrite !comp_trpoly_coef0_eq0 //.
by apply/val_inj; rewrite !rmorphB.
Qed.
Canonical comp_trpoly_additive f (Hf : f \in coef0_is_0) :=
  Additive (comp_trpoly_is_additive Hf).

Fact comp_trpoly_is_linear :
  {in @coef0_is_0 R n, forall f, linear (comp_trpoly f)}.
Proof.
move=> f f0_is_0 a q r.
by rewrite !comp_trpoly_coef0_eq0 // !rmorphD /= !linearZ.
Qed.
Canonical comp_trpoly_linear f (Hf : f \in coef0_is_0) :=
  Linear (comp_trpoly_is_linear Hf).

Fact comp_trpoly_is_rmorphism :
  {in @coef0_is_0 R n, forall f, rmorphism (comp_trpoly f)}.
Proof.
move=> f f0_is_0.
split; first exact: comp_trpoly_is_additive.
split => [g1 g2|]; last exact: comp_trpoly1.
rewrite /comp_trpoly f0_is_0 //.
rewrite val_mul_trpoly /= -trXn_comp_polyl ?(eqP f0_is_0) //.
rewrite rmorphM /= -trXn_mul /=.
by rewrite -[RHS]trpolyK val_mul_trpoly /= trXn_trXn.
Qed.
Canonical comp_trpoly_rmorphism f (Hf : f \in coef0_is_0) :=
...
Lemma comp_trpolyXn i : n != 0%N ->
                        {in coef0_is_0, forall f, \X^+i \So f = f^+ i}.
Proof.
...
Proof. by rewrite coef0_is_0E; case: o => [|o]; rewrite ?coef0 ?coef_poly. Qed.
...
have Xlag0 : trXns o.+1 (mulfX (trXns o.+1 g \So LR)) \in coef0_is_0.
...
apply: lagrinvPl.
rewrite /lagrunit unfold_in coef0_is_0E coef_mulfX /= eqxx /=.
...
Proof.
move=> gU.
case: k i => [|k] [|i] le_ik; rewrite ?expr0 ?subn0 ?coef1 ?mulr0n ?mul0rn //.
move: le_ik; rewrite !ltnS => /andP [le_ki le_in].
have Xg0 : mulfX g^-1 \in coef0_is_0 by rewrite coef0_is_0E coef_mulfX.
...
move=> /(congr1 (fun s => (trpoly _ (s * g ^+ i.+1))`_i)).
rewrite mulrnAl -mulrnAr coef_trpolyMXn subSS ltnNge le_ki le_in /=.
rewrite raddfMn coefMn /= => <-.
rewrite mulr_suml raddf_sum coef_sum /= -/(_`_i.+1).
have Hi : i.+1 < n.+3 by rewrite ltnS (leq_ltn_trans le_in).
rewrite (bigD1 (Ordinal Hi)) //= -/(_`_i.+1).
move: (LGR`_i.+1) => co.
rewrite !linearZ /= -scalerAl coefZ /= rmorphX /= comp_trpolyX //.
rewrite derivX_trpoly /= !mulrnAl raddfMn coefMn /= mulrnAr -mulrnAl.
rewrite -mulrA coef_trpolyM le_in big_ord_recr /=.
...
rewrite deriv_trpolyM deriv_trpolyX mul1r /= deriv_trXns.
...
rewrite mulrnAl -!mulrA raddfMn coefMn /= coef_trpolyMXn le_in.
...
rewrite deriv_trpolyV // /= mulfXE mulNr raddfN mulrN mulNr /= -mulfXE.
have -> : mulfX ((g^`())%trpoly / trXns n (g ^+ 2)) =
          mulfX (g^`())%trpoly / g ^+ 2.
  apply/trpolyP => l Hl; rewrite /= in Hl.
  rewrite coef_mulfX !coef_trpolyM.
  case: l Hl => [_|l].
...
rewrite mulrDl -exprS mulNr coefB /= -!/(_`_d.+1).
apply/eqP; rewrite subr_eq0; apply/eqP.
have := erefl (((g ^+ d.+1)^`())%trpoly`_d / (d.+1)%:R).
rewrite {1}coef_deriv_trpoly -mulr_natr mulrK // => ->.
rewrite derivX_trpoly raddfMn coefMn -mulr_natr mulrK //= -/(_`_d.+1).
have lt_dn1 := leq_trans lt_di le_in.
rewrite mulrC !coef_trpolyM -ltnS lt_dn1.
...
move: f g; case: n => [|m] f g f0_eq0.
  rewrite [f]trpoly_is_cst [g]trpoly_is_cst /trXns trXn_trXn // comp_trpolyC.
...
by rewrite -[LHS]trpolyK !linearZ /= val_exp_trpoly trXn_trXn.
Qed.
...
rewrite big_ord_recl /= fact0 invr1 alg_polyC polyC1.
...
- rewrite coef0_is_0E coef0_trXn coefC /= => /eqP ->.
  by rewrite eq_refl trpolyC0 trpolyC1 exp0.
- rewrite exp_coef0_isnt_0 //.
  move: p0N0; rewrite coef0_is_0E coef0_trXn coefC /= => /negbTE ->.
  by rewrite rmorph0.
Qed.
...
by rewrite -[LHS]trpolyK !linearZ !linearN /= val_exp_trpoly trXn_trXn.
Qed.
...
Proof.
apply/trpolyP => j Hj; rewrite log_coef0_is_1 ?rpred1 // coef0 subrr.
...
Theorem deriv_trpolyX f m :
  (f ^+ m)^`() = f^`() * (trXns n.-1 f) ^+ m.-1 *+ m.
Proof.
elim: m => /= [|m IHm]; first by rewrite expr0 mulr0n -trpolyC1 deriv_trpolyC.
rewrite exprS deriv_trpolyM {}IHm.
rewrite -!mulrnAr mulrCA -mulrDr; congr (_ * _).
rewrite !rmorphX /= leq_pred // => _.
rewrite mulrnAr -exprS; case: m => /= [|m]; rewrite -?mulrS //.
...
Theorem deriv_trpolyXn m :
  m != 0%N ->  {in GRing.unit, forall f,
                     (f ^- m)^`() = - f^`() * (trXns n.-1 f) ^- m.+1 *+ m}.
Proof.
move=> Hm /= f fU; rewrite -exprVn deriv_trpolyX deriv_trpolyV //.
rewrite rmorphV ?leq_pred //= => _.
rewrite !exprVn rmorphX ?leq_pred //= => _.
rewrite -mulrA; congr ((_ * _) *+ m).
rewrite -!exprVn -exprD.
by case: m Hm.
Qed.
...
Theorem deriv_exp f : (exp f)^`() = f^`() * (trXns n.-1 (exp f)).
Proof.
move: f; case: n => /= [| m] f.
...
rewrite rmorphX expr_coef0_is_0 ?coef0_is_0_trXns // scaler0 addr0.
...
apply: eq_bigr => i _.
rewrite !linearZ -scalerCA /= deriv_trpolyX.
rewrite -scaler_nat scalerA /= rmorphX /= -scalerAl; congr (_ *: _).
rewrite factS natrM /= invrM ?fact_unit //.
by rewrite -mulrA [X in _ * X]mulrC divrr // mulr1.
Qed.

Theorem deriv_log f :
  f \in coef0_is_1 -> (log f)^`() = f^`() / (trXns n.-1 f).
Proof.
case: n f => [|m] f.
  rewrite [f]trpoly_is_cst coef0_is_1E coef0_trXn coefC eqxx => /eqP ->.
  by rewrite !deriv_trpoly0p mul0r.
move => f0_is_1.
rewrite log_coef0_is_1 //.
rewrite !raddf_sum /= big_nat.
rewrite (eq_bigr (fun i => f^`() * ((1 - (trXns m f)) ^+ i.-1))) =>
                                                  [|i /andP [hi _]]; last first.
- rewrite linearN linearZ /= deriv_trpolyX /=.
  rewrite -scaler_nat scalerA mulrC divrr; last by case: i hi.
  rewrite scale1r linearD /= deriv_trpoly1 add0r linearN.
  by rewrite mulNr opprK rmorphB /= trXns1.
- rewrite -big_nat /= -mulr_sumr big_add1 /= big_mkord; congr (_ * _).
  ...
  by rewrite -coef0_is_10 coef0_is_1_trXns.
Qed.
...
have {Hlog} : (p / q)^`() = 0.
  ...
  rewrite -eq_divr ?trXns_unitE -?unit_trpolyE ?coef0_is_1_unit //.
...
rewrite mulrAC -mulrA divrr ?trXns_unitE -?unit_trpolyE ?coef0_is_1_unit //.
...
Lemma logV : {in (@coef0_is_1 R n), {morph log : f / f^-1 >-> - f}}.
Proof.
move=> f f0_eq1 /=.
apply exp_inj; rewrite ?rpredN ?log_in_coef0_is_0 //.
rewrite expN ?log_in_coef0_is_0 //.
by rewrite !logK // rpredV.
Qed.

Lemma log_div : {in (@coef0_is_1 R n) &, {morph log : f g / f / g >-> f - g}}.
Proof.
move=> f g f0_eq1 g0_eq1 /=.
...
rewrite -{1}add1n natrD scalerDl scale1r expD ?log_in_coef0_is_0 //.
...
Lemma expr_trpolyNn f m : f \in coef0_is_1 -> f ^^ (-m%:R) = f ^- m.
Proof.
move=> Hf.
